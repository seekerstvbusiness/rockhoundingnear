...
import { US_STATES, GEM_TYPES, SITE_URL } from '@/lib/constants'

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const now = new Date()

  const gemTypeRoutes: MetadataRoute.Sitemap = GEM_TYPES.map((gem) => ({
    url: `${SITE_URL}/gem-types/${gem.toLowerCase().replace(/\s+/g, '-')}`,
    lastModified: now,
  ...
  const staticRoutes: MetadataRoute.Sitemap = [
    ...
    { url: `${SITE_URL}/gem-types`, lastModified: now, changeFrequency: 'weekly', priority: 0.8 },
  ...
  return [...staticRoutes, ...stateRoutes, ...gemTypeRoutes, ...locationRoutes]
}
